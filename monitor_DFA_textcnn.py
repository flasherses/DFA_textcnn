...
with connection.cursor() as cursor:
    while True:
        ...
        # 移除结果中的HTML标签
        md = re.sub(r'<p>|</p>', '', result)
        # 初始化输入文本
        input_text = re.sub(r'\d+|\s+', '', result)
        ...
        if len(input_text) <= 60:
            ...
            # 将输入文本转换为模型可以接受的序列
            new_short_padded_seqs = pad_sequences(tokenizer.texts_to_sequences([input_text]), maxlen=60, padding='post', value=1e-10)
            ...
            if prediction == 1:
                ...
                # 对输入文本进行分词，并对每个词进行预测
                word_predictions = model.predict(pad_sequences(tokenizer.texts_to_sequences(input_text), maxlen=60, padding='post', value=1e-10))
            ...
            # 对平滑分割后的长文本进行中文分词和停用词移除
            input_long_text = list(map(lambda x: remove_stopwords_long(chinese_word_cut_long(x)), input_long_text_smooth_split))
            print(input_long_text)

            # 将长文本转换为模型可以接受的序列，并进行填充
            new_long_padded_seqs = pad_sequences(tokenizer.texts_to_sequences(input_long_text), maxlen=60, padding='post', value=1e-10)
            ...
            if np.any(predictions[:, 1] >= 0.5):
                ...
                for text_smooth_split, text, prediction in zip(input_long_text_smooth_split, input_long_text,
                    ...
                    if prediction[1] >= 0.5:
                        mgw = []  # 初始化敏感词列表
                        # 将文本转换为模型可以接受的序列，并进行填充
                        new_word_padded_seqs = pad_sequences(tokenizer.texts_to_sequences(text), maxlen=60, padding='post', value=1e-10)
                        # 使用模型对文本进行预测
                        ...
                        # 格式化敏感详情
                        formatted_sensitive_details = [f'"{sentence}": 句子敏感度 {sentence_prediction:.2f}, 包含敏感词: {", ".join([f"{word} ({prob:.2f})" for word, prob in words])}' for sentence, words, sentence_prediction in sensitive_details]
